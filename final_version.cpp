...
enum enMainMenuePermissions { eAll = -1,
                             pListClients = 1,
                             pAddNewClient = 2,
                             pDeleteClient= 4,
                             pUpdateClients = 8,
...
void access_denied_message(){
 ...
 bool check_access(enMainMenuePermissions Permission) {
    if (current_user.permissions == enMainMenuePermissions::eAll)return true;
    if ((Permission & current_user.permissions) == Permission) return true;
    else
        return false;
}
string account_num() {
...
void main_program(choice_client choice){
    while(true) {
            if (choice == ::show_list_of_clients) {
                if(check_access(enMainMenuePermissions::pListClients)){
                    showing_clients();
                    go_back_to_main_menu();
                    break;
                }
            ...
            if (choice == ::add_new_client ) {
                if(check_access(enMainMenuePermissions::pAddNewClient)){
                    add_clients_screen();
                    go_back_to_main_menu();
                    break;
                }
            ...
            if (choice == ::find_client_) {
                if(check_access(enMainMenuePermissions::pFindClient)){
            ...
            if (choice == ::update_client_information) {
                if(check_access(enMainMenuePermissions::pUpdateClients)){
                    updating_screen();
                    go_back_to_main_menu();
                    break;
                }
            ...
            if (choice == ::delete_client) {
                if(check_access(enMainMenuePermissions::pDeleteClient)){
                    deleting_screen();
                    go_back_to_main_menu();
                    break;
                }
            ...
            if (choice == ::transactions) {
                if(check_access(enMainMenuePermissions::pTranactions)){
                    transaction_menu();
                    go_back_to_main_menu();
                    break;
                }
            ...
            if (choice == ::manage_user) {
                if(check_access(enMainMenuePermissions::pManageUsers)){
                    manage_user_screen();
                    go_back_to_main_menu();
                    break;
                }
...
stuser_data ConvertLinetoRecord_user_version(string line) {
    ...
    user.permissions=stoi(vUser_data.at(2));
    return user;
}
...
string join_st_user(stuser_data client, string sep = "#//#") {
    ...
    joined += to_string(client.permissions);
    return joined;
}
    ...
    if (file.is_open()) {
        for ( const stuser_data &user : users)
        {
            ...
            if (user.mark_for_update) {
                ...
                updated_user.permissions=permissions_();
...
int permissions_(){
    ...
    char per;
    int permission=0;
    cout<<"\ndo you want to give full permission y/n";
     cin>>per;
     if(per=='y'||per=='Y'){
         return -1;
     }
...
bool adding_screen_validation(stuser_data &user_added,vector<stuser_data>&vUsers){
    for (auto &i:vUsers) {
...
void user_add(stuser_data &user){
    ...
    while(!adding_screen_validation(user,vUsers));
    user.permissions=permissions_();
}
...
void manage_user_logic(choice_manage_users choice){
    while(true) {
...
bool FindUserByUsernameAndPassword(string Username, string Password, stuser_data& User)
{
    ...
    for (stuser_data user : users)
    {
        ...
        if (user.authority == Username && user.password == Password)
        {
            User = user;
            return true;
        }
